...
func TestImportTailMessageFailureFallbacksExactLeavesForeignEventKindUntouched(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	dbPath := filepath.Join(t.TempDir(), "discrawl.db")
	s, err := Open(ctx, dbPath)
	...
	for _, fallback := range []TailMessageFailureFallback{
		{
			EventKind: "create", FailureKind: "timeout",
			GuildID: "g1", ChannelID: "c1", MessageID: "same",
		},
		{
			EventKind: "update", FailureKind: "panic",
			GuildID: "g1", ChannelID: "c1", MessageID: "same",
	...
	imported, err := s.ImportTailMessageFailureFallbacksExact(ctx, []FailureRef{{
	...
	require.NoError(t, err)
	require.Contains(t, string(body), `"event_kind":"update"`)

	var createCount int
	var updateCount int
	require.NoError(t, s.DB().QueryRowContext(ctx, `
		select count(*) from failure_ledger
		where message_id = 'same' and related_id = 'create'
	`).Scan(&createCount))
	require.NoError(t, s.DB().QueryRowContext(ctx, `
		select count(*) from failure_ledger
		where message_id = 'same' and related_id = 'update'
	`).Scan(&updateCount))
...
func TestImportTailMessageFailureFallbacksExactValidatesForeignFallbackBeforeMutation(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	s, err := Open(ctx, filepath.Join(t.TempDir(), "discrawl.db"))
	require.NoError(t, err)
	...
	require.NoError(t, s.PersistTailMessageFailureFallback(TailMessageFailureFallback{
		EventKind: "create", FailureKind: "timeout",
		GuildID: "g1", ChannelID: "c1", MessageID: "selected",
	}))
	dir, err := s.tailMessageFailureFallbackDir()
	require.NoError(t, err)
	corrupt := []byte(`{"version":1}`)
	require.NoError(t, os.WriteFile(
		filepath.Join(dir, committedTailFailureName(corrupt)),
		corrupt,
		0o600,
	))
...
func TestImportTailMessageFailureFallbacksExactRejectsInvalidAllowlist(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	s, err := Open(ctx, filepath.Join(t.TempDir(), "discrawl.db"))
	require.NoError(t, err)
	...
	_, err = s.ImportTailMessageFailureFallbacksExact(ctx, nil)
	require.ErrorContains(t, err, "at least one")
	invalidScope := valid
	invalidScope.Operation = "other"
	_, err = s.ImportTailMessageFailureFallbacksExact(ctx, []FailureRef{invalidScope})
	require.ErrorContains(t, err, "invalid scope")
	incomplete := valid
	incomplete.MessageID = ""
	_, err = s.ImportTailMessageFailureFallbacksExact(ctx, []FailureRef{incomplete})
	require.ErrorContains(t, err, "incomplete")
	_, err = s.ImportTailMessageFailureFallbacksExact(ctx, []FailureRef{valid, valid})
	require.ErrorContains(t, err, "duplicate")
}

func TestImportTailMessageFailureFallbacksExactRollsBackUnavailableBatch(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	dbPath := filepath.Join(t.TempDir(), "discrawl.db")
	s, err := Open(ctx, dbPath)
	...
	require.NoError(t, s.PersistTailMessageFailureFallback(TailMessageFailureFallback{
		EventKind: "create", FailureKind: "timeout",
		GuildID: "g1", ChannelID: "c1", MessageID: "available",
	}))
	...
	available := base
	available.MessageID = "available"
	missing := base
	missing.MessageID = "missing"
	imported, err := s.ImportTailMessageFailureFallbacksExact(
		ctx,
		[]FailureRef{available, missing},
	)
	require.Zero(t, imported)
	...
	entries, err := os.ReadDir(dbPath + tailMessageFailureFallbackDirSuffix)
	require.NoError(t, err)
	require.Len(t, entries, 1)
}

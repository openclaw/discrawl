...
func (t *externalLexicalTokenizer) QueryGroups(ctx context.Context, text string) ([][]string, error) {
	response, err := t.request(ctx, text, true)
	if err == nil && response.Groups == nil {
		return nil, fmt.Errorf("%s helper lacks query groups; rebuild the helper from the current Discrawl source", t.language)
	}
	return response.Groups, err
}

func (t *externalLexicalTokenizer) Tokenize(ctx context.Context, text string) (string, error) {
	response, err := t.request(ctx, text, false)
...
func (t *externalLexicalTokenizer) Identity() string {
	return lexicalFingerprint(t.command.Args...)
}
...
func (t *externalLexicalTokenizer) request(ctx context.Context, text string, query bool) (helperLexicalResponse, error) {
	t.mutex.Lock()
	...
	go func() {
		...
		if err != nil {
			err = fmt.Errorf("write %s tokenizer request: %w", t.language, err)
		} else {
			response, err = t.readResponse()
		}
		result <- struct {
	...
	case output := <-result:
		if output.err != nil {
			return helperLexicalResponse{}, output.err
		}
		if output.response.Error != "" {
			return helperLexicalResponse{}, errors.New(output.response.Error)
		}
		return output.response, nil
	case <-ctx.Done():
		_ = t.command.Process.Kill()
		<-result
		return helperLexicalResponse{}, ctx.Err()
	case <-time.After(30 * time.Second):
		_ = t.command.Process.Kill()
		<-result
		return helperLexicalResponse{}, errors.New("lexical tokenizer response timed out after 30s")
	}
}

...
func TestFailureReplayCandidatesByExactIdentitiesPreserveOrderAndFailClosed(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	s, err := Open(ctx, filepath.Join(t.TempDir(), "discrawl.db"))
	require.NoError(t, err)
	...
	for _, identity := range identities {
		require.NoError(t, s.RecordFailure(ctx, identity, context.DeadlineExceeded))
	}

	candidates, err := s.ListFailureReplayCandidatesByExactIdentities(
		ctx,
		[]FailureRef{identities[1], identities[0]},
		[]string{"g1"},
	)
	require.NoError(t, err)
	require.Equal(
	...
	require.NoError(t, s.ResolveFailureIdentity(ctx, identities[0]))
	_, err = s.ListFailureReplayCandidatesByExactIdentities(
		ctx,
		[]FailureRef{identities[0]},
		[]string{"g1"},
	)
	...
	_, err = s.ListFailureReplayCandidatesByExactIdentities(
		ctx,
		[]FailureRef{identities[1], identities[1]},
		[]string{"g1"},
	)

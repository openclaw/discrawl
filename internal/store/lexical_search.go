...
func (s *Store) searchMessagesMultilingual(
	...
	for _, language := range s.lexicalLanguages() {
		var identity string
		err := s.db.QueryRowContext(ctx, `select coalesce(cursor, '') from sync_state where scope = ?`, lexicalFTSScope(language)).Scan(&identity)
		if err != nil && !errors.Is(err, sql.ErrNoRows) {
			return nil, err
		}
		if identity != s.lexicalTokenizers[language].Identity() {
			return nil, fmt.Errorf("%s lexical index needs rebuilding; run discrawl lexical rebuild with this configuration", language)
		}
	}
	...
	for _, language := range s.lexicalLanguages() {
		query, err := lexicalQuery(ctx, s.lexicalTokenizers[language], opts.Query)
		if err != nil {
...
func lexicalQuery(ctx context.Context, tokenizer LexicalTokenizer, text string) (string, error) {
	groups, err := tokenizer.QueryGroups(ctx, text)
	if err != nil {
		return "", err
	}
	var required []string
	for _, group := range groups {
		if len(group) == 0 {
			return "", errors.New("tokenizer returned an empty query group")
		}
		var alternatives []string
		for _, token := range group {
			if strings.ContainsFunc(token, func(r rune) bool { return unicode.IsLetter(r) || unicode.IsNumber(r) || unicode.Is(unicode.Co, r) }) {
				alternatives = append(alternatives, normalizeFTSQuery(token))
			}
		}
	...
	return strings.Join(required, " AND "), nil
}

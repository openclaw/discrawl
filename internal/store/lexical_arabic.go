...
func (arabicLexicalTokenizer) Identity() string { return lexicalFingerprint("arabic-light-v2") }
...
func (arabicLexicalTokenizer) QueryGroups(_ context.Context, text string) ([][]string, error) {
	var groups [][]string
	for _, word := range splitArabicWords(normalizeArabic(text)) {
		groups = append(groups, tokenizeArabic(word))
	}
	return groups, nil
}

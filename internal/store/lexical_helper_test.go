...
func TestLexicalTokenizerHelperProcess(t *testing.T) {
	...
	for scanner.Scan() {
		...
		response, err := json.Marshal(helperLexicalResponse{Tokens: tokens, Groups: groups})
		if err != nil {
...
func TestLexicalHelperCancellationInterruptsBlockedWrite(t *testing.T) {
	tokenizer, err := startHelperLexicalTokenizerCommand("ko", lexicalHelperCommand("blocked"))
	require.NoError(t, err)
	defer func() { _ = tokenizer.Close() }()
	ctx, cancel := context.WithTimeout(context.Background(), 100*time.Millisecond)
	defer cancel()
	_, err = tokenizer.Tokenize(ctx, strings.Repeat("large request", 100000))
	require.ErrorIs(t, err, context.DeadlineExceeded)
	// A second call must not race the canceled request's scanner goroutine.
	_, err = tokenizer.Tokenize(context.Background(), "next")
	require.Error(t, err)
}

...
type LexicalTokenizer interface {
	...
	QueryGroups(context.Context, string) ([][]string, error)
	Close() error
}

func lexicalFingerprint(parts ...string) string {
	return fmt.Sprintf("%s:%x", lexicalFTSVersion, sha256.Sum256([]byte(strings.Join(parts, "\x00"))))
}

func openLexicalStore(
	ctx context.Context,
	path string,
	...
	if err := store.ensureLexicalFTS(ctx, forceRebuild); err != nil {
		_ = store.Close()
		return nil, err
	}
...
func (s *Store) ensureLexicalFTS(ctx context.Context, force bool) error {
	for _, language := range s.lexicalLanguages() {
		var version sql.NullString
		err := s.db.QueryRowContext(ctx, `
			select cursor from sync_state where scope = ?
		`, lexicalFTSScope(language)).Scan(&version)
		if !force && err == nil && version.String == s.lexicalTokenizers[language].Identity() {
			var exists int
			if err := s.db.QueryRowContext(ctx, `select count(*) from sqlite_schema where name = ?`, lexicalFTSTable(language)).Scan(&exists); err != nil {
				return err
			}
			if exists != 0 {
				continue
...
func (s *Store) rebuildLexicalFTS(ctx context.Context, language string) error {
	...
	// Commit the analyzer identity with its rows, including explicit rebuilds.
	if _, err := tx.ExecContext(ctx, `
		insert into sync_state(scope, cursor, updated_at) values(?, ?, ?)
		on conflict(scope) do update set cursor = excluded.cursor, updated_at = excluded.updated_at
	`, lexicalFTSScope(language), s.lexicalTokenizers[language].Identity(), time.Now().UTC().Format(timeLayout)); err != nil {

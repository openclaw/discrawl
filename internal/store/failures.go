...
// ListFailureReplayCandidatesByExactIdentities returns unresolved failures in
// the supplied identity order. Every identity must exist and remain unresolved.
func (s *Store) ListFailureReplayCandidatesByExactIdentities(
	ctx context.Context,
	identities []FailureRef,
	guildIDs []string,
) ([]Failure, error) {
	if len(identities) == 0 {
		return nil, errors.New("at least one exact failure replay identity is required")
	}
	if len(identities) > maxFailureLimit {
	...
	failures := make([]Failure, 0, len(identities))
	for index, identity := range identities {
		identity = normalizeFailureRef(identity)
		if identity.Operation == "" || identity.Source == "" {
		...
		if identity.GuildID == "" ||
			...
			identity.RelatedID == "" {
			return nil, errors.New("exact failure replay identity is incomplete")
		}
		if len(allowedGuilds) > 0 {
			if _, ok := allowedGuilds[identity.GuildID]; !ok {
				return nil, errors.New("exact failure replay identity is outside the guild scope")
			}
		}
		...
		if _, ok := seen[key]; ok {
			return nil, errors.New("exact failure replay identities contain a duplicate")
		}
		seen[key] = struct{}{}
		...
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf(
				"exact failure replay identity %d is unavailable",
				index+1,
			)
		}
		if err != nil {
			return nil, fmt.Errorf("load exact failure replay identity: %w", err)
		}
		failure.FirstSeenAt = parseTime(firstSeen)
		failure.LastSeenAt = parseTime(lastSeen)
		failure.ResolvedAt = parseTime(resolved)

...
func TestHelperQueryProtocolReturnsSeparateGroups(t *testing.T) {
	tokenizer, err := startKiwiLexicalTokenizerCommand(lexicalHelperCommand("ready"))
	require.NoError(t, err)
	defer func() { _ = tokenizer.Close() }()
	groups, err := tokenizer.QueryGroups(context.Background(), "오늘 저녁먹음 기록")
	require.NoError(t, err)
	require.Equal(t, [][]string{{"오늘"}, {"저녁"}, {"먹"}, {"음"}, {"기록"}}, groups)
}

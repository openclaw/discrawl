...
func TestCollectDiscordPublicCountsAndUnknowns(t *testing.T) {
	cases := []struct {
		...
		{"fractional count", 200, `{"guild":{"id":"123"},"approximate_member_count":1.5,"approximate_presence_count":3}`, nil, new(3.0)},
		{"malformed presence", 200, `{"guild":{"id":"123"},"approximate_member_count":12,"approximate_presence_count":"bad"}`, new(12.0), nil},

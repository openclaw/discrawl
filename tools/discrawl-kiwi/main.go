...
func main() {
	...
	for scanner.Scan() {
		...
		if input.Query {
			groups, err := queryGroups(analyzer, input.Text)
			if err != nil {
				writeResponse(response{Error: err.Error()})
			} else {
				writeResponse(response{Groups: groups})
			}
			continue
		}
...
func tokenize(analyzer kiwiAnalyzer, text string) ([]string, error) {
	...
	for _, token := range results[0].Tokens {
		switch token.Tag {
		case kiwi.POS_SF, kiwi.POS_SP, kiwi.POS_SS, kiwi.POS_SSO, kiwi.POS_SSC, kiwi.POS_SE, kiwi.POS_SO, kiwi.POS_SW, kiwi.POS_SB:
			continue
		}
...
func queryGroups(analyzer kiwiAnalyzer, text string) ([][]string, error) {
	results, err := analyzer.Analyze(text, kiwi.WithTopN(1))
	if err != nil {
		return nil, fmt.Errorf("analyze Korean query: %w", err)
	}
	if len(results) == 0 {
		return nil, errors.New("Kiwi returned no analysis")
	}
	...
	for _, token := range results[0].Tokens {
		...
		if strings.HasPrefix(tag, "J") || strings.HasPrefix(tag, "E") {
			continue
		}
		switch token.Tag {
		case kiwi.POS_SF, kiwi.POS_SP, kiwi.POS_SS, kiwi.POS_SSO, kiwi.POS_SSC, kiwi.POS_SE, kiwi.POS_SO, kiwi.POS_SW, kiwi.POS_SB:
		...
		form := strings.ToLower(strings.TrimSpace(token.Form))
		if form != "" {
			groups = append(groups, []string{form})
		}
	}
	return groups, nil
}

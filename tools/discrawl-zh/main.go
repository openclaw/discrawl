...
func main() {
	...
	for scanner.Scan() {
		...
		if input.Query {
			writeResponse(response{Groups: queryGroups(&seg, input.Text)})
		} else {
...
func queryGroups(seg *gse.Segmenter, text string) [][]string {
	groups := make([][]string, 0)
	for _, token := range unique(seg.Cut(text, true)) {
